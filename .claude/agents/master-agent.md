...
- **Classify the task and recommend a model** before anything else — see
  ...
  warrants: implementation, tests, migrations, static-analysis fixes, and docs →
...
- Read the root `CLAUDE.md`, and `docs/architecture.md` for the layer you are touching.
...
5. **The OpenAPI document is the published contract** — every status code annotated, no
...
| Tests (unit / integration / architecture) | [`test-engineer.md`](test-engineer.md) |
...
| API surface changed | — | [`api-change.md`](../workflows/api-change.md) |
...
checklist, domain entity, API controller.

## Definition of done (per task)

The slice stays vertically complete; `dotnet build` (warnings as errors) and every test green;
any schema change shipped as a reviewed migration; the OpenAPI document complete and
`docs/api/` in sync; SonarQube clean (zero Blocker/Critical/Major); results summarised — **and the push
