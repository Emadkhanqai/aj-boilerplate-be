...
| `agents/` | Specialist subagent definitions — orchestration, backend, review, security, tests, gate. |
...
| `templates/` | Copy-paste starting points — ADR, pull request, migration checklist, domain entity, API controller. |
...
| `../.mcp.json` | MCP servers: SonarQube, Context7, and the cloud pair. |
...
handoffs), and the root `CLAUDE.md`.
...
| `claude-code-workflows` | `wshobson/agents` | `backend-development`, `unit-testing`, `database-migrations`, `security-scanning` |
...
`claude-code-workflows` ships 95 plugins; four stack-relevant ones are enabled to keep context
...
| `auto-format.sh` | PostToolUse `Edit\|Write` | no | Formats **only** the edited file: `.cs` → `dotnet format`, scoped to the nearest `.csproj`. Everything else is left alone; `.editorconfig` is the single source of style truth. |
...
| `run-affected-tests.sh` | PostToolUse `Edit\|Write` | no (exit 1 on failure) | Runs the touched project's tests immediately — `dotnet test` on the edited test project, or on the unit-test project when a source file changed. Silent when nothing is affected. **Never exits 2**: it surfaces a failure without cancelling the edit. |
...
```
/qa          # everything local: build, format, test, dependency audit, secret scan, Sonar
...
```bash
dotnet build --no-incremental && dotnet format --verify-no-changes && dotnet test
...
3. Link it from the agents that need it (`agents/backend-agent.md`) and from
   `commands/review.md` if reviewers should check it.
