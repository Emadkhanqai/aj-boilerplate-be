...
- **Findings from a scanner run against the sample `Item` entity.** It is demonstration
  scaffolding and is meant to be deleted.
...
| Gate | Where |
|---|---|
| **Gitleaks** secret scanning | `backend-ci.yml`, and locally in `.claude/hooks/secret-scan.sh`. Config in [`.github/gitleaks.toml`](.github/gitleaks.toml) |
| **CodeQL** code scanning | `backend-ci.yml` (C#), `security-and-quality` query suite |
