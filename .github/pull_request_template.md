...
<!--
  REQUIRED for any change a consumer of this API can observe: a new or changed
  endpoint, a changed response shape, a new error code, a changed status code, a
  new required header. Not required otherwise — write "n/a — no contract change"
  and move on.

  Paste the real request and the real response, before and after. A description of
  what the API now returns is not evidence of what it returns.

  `docs/api/openapi.json` is committed and checked in CI; if it moved, say what
  moved and why, because that diff is the thing every consumer reads.
-->
