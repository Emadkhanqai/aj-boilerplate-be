...
**Prerequisites:** .NET SDK 10, Docker, and git. That is the whole list.

```bash
git clone https://github.com/<your-org>/aj-boilerplate-be.git
...
docker compose up -d --build                      # SQL Server, Redis, and the API
docker compose --profile tools run --rm migrate   # apply the EF Core migrations
```

That brings up the API on <http://localhost:8080> with its OpenAPI UI at `/swagger`, backed
by SQL Server and Redis. Migrations are a separate step on purpose — applying a schema
change is a controlled action, not an application-startup side effect. `.env.example`
documents every environment variable the repository reads and what each is for.

Running the API from the SDK instead of from a container, and the rest of the local loop,
is in [README.md](README.md), which also lists the repository variables and secrets CI
needs.
...
| Unit tests | `AjBoilerplate.UnitTests` |
| Architecture tests | `AjBoilerplate.ArchitectureTests` — layer dependency direction |
| Integration tests | `AjBoilerplate.IntegrationTests` against a real SQL Server (Testcontainers) |
| API contract | `./scripts/generate-openapi.sh --check` — the committed OpenAPI document matches the code |
| Static analysis | SonarQube, CodeQL |
| Dependencies | `dotnet list package --vulnerable`, including transitives, failing at Moderate |
| Secrets | Gitleaks |
...
dotnet test  AjBoilerplate.slnx
./scripts/generate-openapi.sh --check
```
...
Analysis settings live in `SonarQube.Analysis.xml`, which is what the SonarScanner for
.NET actually reads — it ignores `sonar-project.properties`, and that file carries the
project identity the hook and the IDE resolve instead. Both the hook and CI pass the same
XML to the scanner, which is what makes a local scan and a CI scan agree.
...
`domain`, `infrastructure`, `infra`, `harness`, `deps`.
...
fix(api): stop the items endpoint paging past the last row
...
- **Document it in `.env.example` section 9 and in the table above** in the same pull
  request.
...
This boilerplate is published as three repositories — `aj-boilerplate-fs` (the source of
truth), `aj-boilerplate-be` (this one, backend only), and `aj-boilerplate-fe`. **All work
originates in `aj-boilerplate-fs`.** This repository is an output, regenerated from that
tree by a derivation script; a change committed here is overwritten by the next
derivation and leaves no trace of why it was made.

So the practical instruction is short: **make the change in `aj-boilerplate-fs` and let it
be derived down.** Everything above — the branch flow, the gate, the commit convention —
describes how a change is made there, and applies unchanged to work that lands here. An
issue opened against this repository is welcome and will be acted on upstream.

The three repositories share a version number and are tagged together; the release and
tagging convention is in [CHANGELOG.md](CHANGELOG.md). The background is
